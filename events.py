...
def delete_event(event_id):
    ...
    try:
        ...
        logger.error(f"Failed to delete event with ID: {event_id}, Error: {str(e)}")
...
def forward_reports_to_slack():
    ...
    try:
        ...
        if "workspaceName" not in data or "channelName" not in data:
            logger.error("Both 'workspaceName' and 'channelName' must be provided in the request body.")
        ...
        response = requests.post("https://95lo0apboj.execute-api.us-east-1.amazonaws.com/dev", json=postPayload)
        ...
        if response.status_code == 200:
            ...
            logger.error(f"Failed to forward reports to Slack. Status code: {response.status_code}, Response: {response.json()}")
